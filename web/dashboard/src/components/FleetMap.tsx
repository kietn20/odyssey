...
const FleetMap: React.FC<FleetMapProps> = ({
	...
	const waypointPositions: LatLngExpression[] = waypoints.map((wp) => [
		wp.latitude,
		wp.longitude,
	]);
	...
	return (
		<div className="map-wrapper">
			...
			<div className="map-tools-sidebar">
				<div className="map-tool-icon" title="Map Settings">
					⚙️
				</div>
				<div className="map-tool-icon" title="Layers">
					🗺️
				</div>
				<div className="map-tool-icon" title="Draw Range">
					📏
				</div>
				<div className="map-tool-icon" title="Polygon Select">
					🔷
				</div>
				<div className="map-tool-icon" title="Add Waypoint">
					📍
				</div>
			</div>
					...
					<div className="waypoint-list">
						{waypoints.map((wp, i) => (
							<div key={i} className="waypoint-item">
								{i + 1}: {wp.latitude.toFixed(4)},{" "}
								{wp.longitude.toFixed(4)}
							</div>
						))}
					</div>
					<div className="waypoints-actions">
						<button className="btn-clear" onClick={onClearMission}>
							Clear
						</button>
						<button className="btn-save" onClick={onSaveMission}>
							Save Mission
						</button>
					</div>
				</div>
			...
			>
				...
				{waypoints.length > 1 && (
					<Polyline
						pathOptions={{ color: "#0088ff", weight: 3 }}
